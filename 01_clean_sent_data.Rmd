...
This particular analysis makes extensive use of ```data.table``` and ```ggplot2```.
...
MS <- fread(file.path(raw_path, "mixed_submissions.csv"))
...
for (conf in names(MS)[-1]) { 
  ...
  write.csv(na.omit(MS[, ..cols]),
            file = file.path(data_path, conf, ".csv")
            )
}
# make an list of submission day_prior for each abstract in Perth_2019
MS_days <- MS[!is.na(Perth_2019), rep(day_prior, Perth_2019)]
# load the Perth_2019 abstracts
P19 <- fread("Perth_2019_subs.csv")
setnames(P19, c("ID", "org", "city", "state", "country", "code", 
                "theme","sub_theme", "paper", "conference"))

# not quite enough submission days in the MS sheet, 
# so assume the last few were all submitted on the last day
